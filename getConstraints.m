...
%
    ...
    for i = 1:length(obstacles)
        ...
        if (pAblen^2-((vRel(1)*pAb(1)+vRel(2)*pAb(2))^2)/sum(vRel.^2))<=r^2
        ...
        % Refer paper for more details
           ...
           % c(end+1) =  -1*(r - pAblen - 1/(pAblen)* (- pAb(1)*agent.velocity(1)- pAb(2)*agent.velocity(2) - pAb(1)*obstacles(i).velocity(1)-pAb(2)*obstacles(i).velocity(2) +2*(pAb(1)*control(1)+pAb(2)*control(2) ) ));
        end
